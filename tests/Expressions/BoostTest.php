...
it('boosts a string query')
    ...
    ->toBeExpression("paradedb.boost(factor => 1.5, query => paradedb.parse(query_string => 'description:shoes'))");

it('boosts a builder query')
    ->expect(new Boost(Query::string()->where('description', 'shoes'), 2))
    ->toBeExpression("paradedb.boost(factor => 2, query => paradedb.parse(query_string => 'description:shoes'))");
