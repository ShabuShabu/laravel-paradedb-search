...
it('boosts a string query')
    ...
    ->toBeExpression("paradedb.boost(boost => 1.5, query => paradedb.parse(query_string => 'description:shoes', lenient => NULL::boolean, conjunction_mode => NULL::boolean))");

it('boosts a builder query')
    ->expect(new Boost(Query::string()->where('description', 'shoes'), 2))
    ->toBeExpression("paradedb.boost(boost => 2, query => paradedb.parse(query_string => 'description:shoes', lenient => NULL::boolean, conjunction_mode => NULL::boolean))");
