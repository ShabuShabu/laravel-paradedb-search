...
it('generates the correct regex pattern tokenizer')
    ->expect(new RegexPattern('description', '(?i)\bh\w*'))
    ->toBeExpression("\"description\"::pdb.regex_pattern('(?i)\bh\w*')");

it('can be used as a type')
    ->expect((new RegexPattern('description', '(?i)\bh\w*'))->useAsType())
    ->toBeExpression("\"description\" pdb.regex_pattern('(?i)\bh\w*')");

it('generates the correct regex pattern tokenizer with filters')
